...
s_account="sonypicturesglobal,sonypicturesus"
var s=s_gi(s_account)

/* last update 2013-02-27 dchen */
s.version = "2.1.7 - US";

/************************** CONFIG SECTION **************************/

s.dynamicAccountSelection=true
s.dynamicAccountList="sonypicsdev=stage.sonypictures.com,spindev.sonypictures.com,devshare.sonypictures.com,external.sonypictures.com,launch.sonypictures.com,launch.sonypictures.jp,launch.sonypictures.co.uk,launch.sonypictures.com.au,test.sonypictures.com,test.sonypictures.jp,test.sonypictures.co.uk,test.sonypictures.com.au,launchdev.sonypictures.com,dev.sonypictures.com,stagenew.sonypictures.com"


s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,sit,wav,mp3,mov,mpg,avi,m4v,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx"
s.linkInternalFilters="javascript:,sonypictures.com,wheeloffortune.com,jeopardy.com,theyoungandtherestless.com,daysofourlives.com,bloodplustv.com,canterburyslawtv.com,judgehatchett.com,judgekarentv.com,judgedavidyoung.com,judgemarialopez.com,kingofqueens.com,myboystv.com,marriedwithchildren.com,thenannytv.com,rulesofengagement-tv.com,dawsonscreek.com,theshieldtv.com,rescuemetv.com,beakmansworldtv.com,spideytv.com"
...
function s_doPlugins(s) {
	...
	if (typeof s.linkName != 'undefined' && (typeof sCode[s.linkName.split('(')[0]] != 'undefined' || s.linkName == 'change_state')) {
	} else {
		s.events=null;
	}
	...
	if(!s.campaign) {
		s.campaign=s.getQueryParam('hs308');
	...
	if(!s.server) s.server=location.hostname;

	s.events=s.apl(s.events,'event2',',',2);
	if(s.products !=null) s.events=s.apl(s.events,'event14',',',2);

	s.today=new Date();
	s.prop7=s.getTimeParting('h','-8',s.today.getFullYear()); 
	s.prop8=s.getTimeParting('d','-8',s.today.getFullYear()); 
	s.prop9=s.getTimeParting('w','-8',s.today.getFullYear()); 

	s.prop13=s.getFlash('s_fv');

	s.partnerURL=s.fullPartnerURL=s.exitLinkHandler();
	if (s.partnerURL != "") {
		s.buyPartnerParam=s.getQueryParam('lohs296,hs296','',s.partnerURL);
		s.otherPartnerParam=s.getQueryParam('lohs320,hs320','',s.partnerURL);
		if(s.buyPartnerParam || s.otherPartnerParam){
			s.partnerURL=s.partnerURL.substring(s.partnerURL.search("//")+2,s.partnerURL.length);
			if (s.partnerURL.search("/")==-1){
				s.partnerURL=s.partnerURL.substring(0,s.partnerURL.length);
			}else{    
				s.partnerURL=s.partnerURL.substring(0,s.partnerURL.search("/"));
			}
			s.linkTrackVars="eVar6,eVar13,eVar14,events";
			s.eVar6=s.fullPartnerURL;
			s.eVar14=s.partnerURL;
			if (s.buyPartnerParam) {
				s.eVar13=s.buyPartnerParam;
				s.linkTrackEvents="event16";
				s.events=s.apl(s.events,"event16",",",2);
			}else{
				s.eVar13=s.otherPartnerParam;
	...
	if (!s.eVar12){
		s.eVar12=s.getQueryParam('lohs317,hs317');
		if(s.eVar12) {
	...
	if(s.channel&&!s.eVar3) s.eVar3=s.channel;
	if(s.prop3&&!s.eVar23) s.eVar23=s.prop3;
	...
	if(s.prop13&&!s.eVar15) s.eVar15=s.prop13;

	s.socialPlatforms('eVar26');
...
s.visitorNamespace="sonypictures"
s.trackingServer="omn.sonypictures.com"
s.trackingServerSecure="somn.sonypictures.com"
s.dc="112"
s.vmk="480A648D"
...
+"'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar27';s.Integrate"
...
function change_state(playStatus){
    if (typeof s != "undefined") {
		...
		for (i=0;i<omni_event_match.length;i++) {
			...
			switch(omni_event_number) {
				...
				break;
				case 111: 
					s.linkTrackVars = 'eVar3,eVar23,eVar4,eVar5,eVar6,prop3,prop4,prop5,prop11,events';
				...
				break;
				case 233: 
					s.linkTrackVars = 'eVar3,eVar23,eVar4,eVar5,eVar6,eVar13,prop3,prop4,prop5,prop11,events';
					s.linkTrackEvents = 'event16';
					s.events = 'event16';
					s.eVar6 = omni_event_string;
					s.eVar13 = omni_event_string;
					s.tl(s, 'o', 'change_state');	
				...
				break;
				case 317:
					s.linkTrackVars = 'eVar3,eVar23,eVar4,eVar5,eVar6,eVar12,prop3,prop4,prop5,prop11,events';
					s.linkTrackEvents = 'event4';
					s.events = 'event4';
					s.eVar6 = omni_event_string;
					s.eVar12 = omni_event_string;
					s.tl(s, 'o', 'change_state');
...
sCode = {
	clear: function (methodName, customVars) {
		this.preservedVars[methodName] = {};
		for (customVar in customVars) {
			this.preservedVars[methodName]['s.' + customVars[customVar]] = s[customVars[customVar]];
			s[customVars[customVar]] = null;
		}
	},
	
	restore: function (methodName, customVars) {
		for (customVar in customVars) {
			s[customVars[customVar]] = this.preservedVars[methodName]['s.' + customVars[customVar]];
		}
	},
	...
	trackPageView: function(pageName, pageType)
	{

		// add current pageName to s object
		s.current_pageName = s.pageName;

		
		var current_prop5 = s.prop5;
		var section = current_prop5.substring(current_prop5.lastIndexOf(':') + 1);

		// get the path without file name
		var path = s.pageName.slice(0, (s.pageName.lastIndexOf(':')+1));

		
		if(pageType === '' || pageType === 'blank') {
			s.pageName = path + pageName;

		} else if(typeof pageType != 'undefined') {
			s.pageName = path + pageType + ":" + pageName;
			s.eVar5 = s.prop5 = s.channel + ":" + pageType;
		...
		if (typeof pageType != 'undefined') {
			s.eVar5 = s.prop5 = current_prop5;
		}

		s.events = null;

		//reset s.pageName
		s.pageName = s.current_pageName;
	...
	trackImpression: function(adID)
	{
		...
		s.linkTrackVars = 'eVar3,eVar23,eVar4,eVar12,prop3,prop4,prop5,prop11,events';
		s.linkTrackEvents = 'event14';
		s.events = 'event14';
		s.eVar12 = s.prop3 + ":" + adID;
	
		s.tl(s, 'o', 'trackImpression(\'' + adID + '\')');
	...
	trackAdClick: function (targetURL, adID)
	{
		...
		s.products=';'+adID;
		s.linkTrackVars = 'eVar6,eVar13,eVar14,products,prop3,prop4,prop5,prop11,events';
		s.linkTrackEvents = 'event15'; 
 		s.events = 'event15'; 
		s.eVar6 = targetURL;
		s.eVar13 = adID;
		s.eVar14 = targetURL.split("/")[2];
	
		s.tl(s, 'o', 'trackAdClick(\'' + targetURL + '\', \'' + adID + '\')');
	...
	trackOutboundClick: function (targetURL, adID)
	{
		var pageName = s.pageName;
		s.pageName = null;
	
		s.linkTrackVars = 'eVar3,eVar23,eVar4,eVar6,eVar13,eVar14,prop3,prop4,prop5,prop11,events';
		s.linkTrackEvents = 'event15';
		s.events = 'event15';
		s.eVar6 = targetURL;
		s.eVar13 = s.prop3 + ":" + adID;
		s.eVar14 = targetURL.split("/")[2];
	
		s.tl(s, 'o', 'trackOutboundClick(\'' + targetURL + '\', \'' + adID + '\')');
	...
	trackOutboundClickToBuy: function (targetURL, adID)
	{
		var pageName = s.pageName;
		s.pageName = null;
		...
		s.tl(s, 'o', 'trackOutboundClickToBuy(\'' + targetURL + '\', \'' + adID + '\')');
	...
	trackFeaturedContentClick: function (targetURL, adID)
	{
		var pageName = s.pageName;
		s.pageName = null;
	...
	trackVideo: function (videoName, state)
	{
		var pageName = s.pageName;
		s.pageName = null;
		...
		switch(state) {
			...
			//-- begin -- new segments added 6/5/2012 --//
			case 'segment1':
				s.eVar22 = '1:segment1';
				_events = 'event18, event7';
				break;
			case 'segment2':
				s.eVar22 = '2:segment2';
				_events = 'event18, event7';
			...
			case 'replay':
				...
				break;
			default:
				//alert('please use a valid video state for trackVideo: start, end, replay');
				return;
				break;
		}

		s.linkTrackVars = 'eVar22,eVar3,eVar23,eVar4,eVar5,eVar6,eVar21,prop3,prop4,prop5,prop11,events';
		s.linkTrackEvents = _events;
		s.events = _events;
		s.eVar21 = s.prop3 + ':' + videoName;
	
		s.tl(s, 'o', 'trackVideo(\'' + videoName + '\', \'' + state + '\')');
	...
	trackMediaOpen: function (mediaName, mediaLength, mediaPlayerName)
	{
		var pageName = s.pageName;
		s.pageName = null;

		mediaName =  s.prop3 + ':' + mediaName;

	
		var clearVars = ['eVar3', 'prop3', 'eVar23', 'eVar4', 'prop4', 'eVar5', 'prop5', 'eVar11', 'prop11'];
		this.clear('trackMediaOpen', clearVars);
		...
		this.restore('trackMediaOpen', clearVars);
	},
	
	
	trackMediaPlay: function (mediaName, mediaOffset)
	{
		var pageName = s.pageName;
		s.pageName = null;
		...
		var clearVars = ['eVar3', 'prop3', 'eVar23', 'eVar4', 'prop4', 'eVar5', 'prop5', 'eVar11', 'prop11'];
		this.clear('trackMediaPlay', clearVars);
		...
		this.restore('trackMediaPlay', clearVars);

	},

	
	trackMediaStop: function (mediaName, mediaOffset)
	{
		var pageName = s.pageName;
		s.pageName = null;
		...
		var clearVars = ['eVar3', 'prop3', 'eVar23', 'eVar4', 'prop4', 'eVar5', 'prop5', 'eVar11', 'prop11'];
		this.clear('trackMediaStop', clearVars);
		
		s.Media.stop(mediaName,mediaOffset);
		
		s.pageName = pageName;
		
		this.restore('trackMediaStop', clearVars);
	},
	
	
	trackMediaClose: function (mediaName)
	{
		var pageName = s.pageName;
		s.pageName = null;
		...
		var clearVars = ['eVar3', 'prop3', 'eVar23', 'eVar4', 'prop4', 'eVar5', 'prop5', 'eVar11', 'prop11'];
		this.clear('trackMediaClose', clearVars);
		
		s.Media.close(mediaName);
		
		s.pageName = pageName;
		
		this.restore('trackMediaClose', clearVars);
	},

	trackDownload: function (downloadName)
	{
		var pageName = s.pageName;
		s.pageName = null;
		s.linkTrackVars = 'eVar3,eVar23,eVar4,eVar6,prop3,prop4,prop5,prop11,events';
		s.linkTrackEvents = 'event11';
		s.events = 'event11';
		s.eVar6 = s.prop3 + ":" + downloadName;
		s.tl(s, 'o', 'trackDownload(\'' + downloadName + '\')');
		s.pageName = pageName;
	...
	trackGame: function (gameName, gamePosition)
	{
		...
		s.eVar6 = s.prop3 + ":" + gameName;
		
		s.tl(s, 'o', 'trackGame(\'' + gameName + '\', \'' + (typeof gamePosition != 'undefined' ? gamePosition : '') + '\')');
		s.pageName = pageName;
